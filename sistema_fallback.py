#!/usr/bin/env python3
"""
Sistema de Upload de Imagens Excel - Versão com Fallback
Versão com SSH, FTP e Google Drive como opções
"""
...
<head>
    ...
    <title>🚀 Sistema Upload Excel - Com Fallback</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Versão com Fallback - SSH, FTP e Google Drive</p>
        </div>
        ...
        <div class="method-selector">
            ...
            <div class="method-option">
                <input type="radio" id="ssh" name="method" value="ssh" checked>
                <label for="ssh">SSH (Porta 65002) - Recomendado</label>
            </div>
            <div class="method-option">
                <input type="radio" id="ftp" name="method" value="ftp">
                <label for="ftp">FTP (Porta 21) - Alternativo</label>
            </div>
        </div>
        ...
        <div class="steps" id="steps" style="display: none;">
            ...
            <div class="step" id="step3">🔌 Conectando</div>
...
def test_ssh_connection():
    ...
    try:
        debug_info = []
        debug_info.append(f"🔌 Testando SSH: {SSH_HOST}:{SSH_PORT}")
        ...
        if result == 0:
            ...
            ssh.connect(SSH_HOST, SSH_PORT, SSH_USER, SSH_PASS, timeout=5)
            sftp = ssh.open_sftp()
            sftp.close()
            ssh.close()
            
            debug_info.append(f"✅ SSH funcionando")
...
def test_ftp_connection():
    ...
    try:
        debug_info = []
        debug_info.append(f"🔌 Testando FTP: {FTP_HOST}:{FTP_PORT}")
...
def create_app():
    ...
    def index():
        ...
        # Testa conectividade
        ssh_working, ssh_debug = test_ssh_connection()
        ftp_working, ftp_debug = test_ftp_connection()
        
        ssh_status = "✅ Conectado" if ssh_working else "❌ Sem conexão"
        ftp_status = "✅ Conectado" if ftp_working else "❌ Sem conexão"
        
        if ssh_working:
            recommended_method = "SSH (Porta 65002)"
        elif ftp_working:
            recommended_method = "FTP (Porta 21)"
        else:
    ...
    def health():
        ...
        try:
            ...
            health_data = {
                "status": "ok",
                "message": "Sistema com Fallback funcionando",
                "timestamp": time.time(),
    ...
    def upload():
        ...
        try:
            method = request.form.get('method', 'ssh')
...
def upload_via_ftp(images, debug_info):
    ...
    try:
        debug_info.append(f"🔌 Conectando ao FTP: {FTP_HOST}:{FTP_PORT}")
        ftp = ftplib.FTP()
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Iniciando Sistema com Fallback...")
    print("🚀 Iniciando Sistema com Fallback...")
