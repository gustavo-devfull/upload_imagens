#!/usr/bin/env python3
"""
🔍 DETECTOR CUSTOMIZADO DE IMAGENS EXCEL
Detector que funciona com imagens embedadas que o openpyxl não consegue acessar
"""
...
class DetectorImagensCustomizado:
    """Detector customizado que acessa imagens via XML"""
    ...
    def detectar_imagens_embedadas(self, arquivo: str, target_columns: List[str] = ['H'], 
                                 start_row: int = 4, ref_column: str = 'A') -> List[Dict]:
        """
        Detecta imagens embedadas acessando diretamente os XMLs do Excel
        ...
        self.log_debug(f"🔍 Iniciando detecção customizada de imagens...")
        ...
        try:
            with zipfile.ZipFile(arquivo, 'r') as zip_file:
                # Passo 1: Ler drawing1.xml para obter posições das imagens
                ...
                # Encontrar todos os OneCellAnchor
                        ...
                        if col_elem is not None and row_elem is not None:
                            ...
                            if col_letter in target_columns and row >= start_row:
                                ...
                                if ref_value and ref_value.upper() not in ['TOTAL', 'SUBTOTAL', '']:
                                    # Procurar por blipFill para obter ID da imagem
                                    blip_fill = anchor.find('.//{http://schemas.openxmlformats.org/drawingml/2006/picture}blipFill')
...
def testar_detector_customizado(arquivo: str):
    """Testa o detector customizado"""
    
    print(f"\n🔍 TESTE DO DETECTOR CUSTOMIZADO")
    print(f"📁 Arquivo: {arquivo}")
    print("=" * 70)
    
    detector = DetectorImagensCustomizado(debug_mode=True)
    images = detector.detectar_imagens_embedadas(arquivo, target_columns=['H'], start_row=4, ref_column='A')
    ...
    if images:
        ...
        print(f"\n✅ SUCESSO! Detector customizado funcionando!")
...
def main():
    ...
    print("🔍 DETECTOR CUSTOMIZADO DE IMAGENS EXCEL")
    print("=" * 70)
    
    arquivo = "produtos_novos.xlsx"
    testar_detector_customizado(arquivo)
